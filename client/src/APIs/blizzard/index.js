...
export const getItemsBaseData = async ({ clientAuthToken, start, results }) => {
  ...
  const field = `&id=[${start},${end}]&item_class.name.en_US=Armor || Weapon`;
...
// export const getWeaponBaseData = async ({ clientAuthToken, start, results }) => {
//   console.log(`Starting range ${start} to ${start + 999}.`)
//   const end = start + 999;
//   const searchQuerybase = "https://us.api.blizzard.com/data/wow/search/item?namespace=static-classic-us&locale=en_US";
//   const field = `&id=[${start},${end}]&item_class.name.en_US=Weapon`;
//   const searchQuery = `${searchQuerybase}${field}&_pageSize=1000&orderby=id`;
//   const getWeaponBaseDataAxios = axios.create({
//     baseURL: searchQuery,
//     headers: {
//       Authorization: `Bearer ${clientAuthToken}`,
//     }
//   });
//   const response = await getWeaponBaseDataAxios.get();
...
//     start = start + 1000
//     results.push(itemsArray)
//     await getWeaponBaseData({ clientAuthToken, start, results })
//   }
